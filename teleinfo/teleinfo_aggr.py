...
#where T1.counterId=cid2 and d1 = date(d2, '-1 day') group by date(T1.date), T1.counterId;
    ...
    query_select1 = "SELECT date(T1.date) as d1, T1.counterId, m2, m2 - max(T1.indexBase) as diff "
    ...
    query_where = "WHERE T1.counterId=cid2 and d1 between '%s' and '%s' and d1=date(d2, '-1 day') "
...
def main():
    ...
    if args.dateStart is not None:
        ...
        args.dateStart = (datetime.date.today() - datetime.timedelta(1)).strftime("%Y-%m-%d")
        
    if args.dateEnd is not None:
        if not re.match("\d{4}-\d{2}-\d{2}", args.dateEnd):
            parser.error("End date format must be yyyy-MM-dd")
    else:
        args.dateEnd = datetime.date.today().strftime("%Y-%m-%d")
